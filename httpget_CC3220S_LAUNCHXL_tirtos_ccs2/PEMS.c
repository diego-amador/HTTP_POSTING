...
void update_values(){
    ...
    case 2:
        cursor_moveto(1, 14);
        sprintf(buffer, "%03d",minT);
        lcd_string(buffer);

        cursor_moveto(2,14);
        sprintf(buffer, "%03d",maxT);
        lcd_string(buffer);
    ...
    case 4:
        cursor_moveto(1, 13);
        sprintf(buffer, "%03d",minC);
        lcd_string(buffer);

        cursor_moveto(2,13);
        sprintf(buffer, "%03d",maxC);
        lcd_string(buffer);
...
void *mainThread(void *arg0){
    ...
    GPIO_setConfig(ENTER, GPIO_CFG_IN_PU | GPIO_CFG_IN_INT_HIGH);
    GPIO_setConfig(UP, GPIO_CFG_IN_PU | GPIO_CFG_IN_INT_HIGH);
    GPIO_setConfig(DOWN, GPIO_CFG_IN_PU | GPIO_CFG_IN_INT_HIGH);
...
//    turn_AC_ON();
    ...
    update_values();
sleep(5);
 post((int)Temp,(int)Hum,(int)CO);


//    post((int)param,(int)CO);
    sleep(10);
...
//***********Thresholds Module***********//
void thresholdMax(uint8_t i){
    //add update value where necesary
    if(screen == 2){
